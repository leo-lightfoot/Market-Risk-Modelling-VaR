...
file_path = os.path.join(project_root, "Raw_Data", "combined_data_gold.csv")
...
df["Date"] = pd.to_datetime(df["Date"])  # Should now be in YYYY-MM-DD format
...
# Interaction Terms
log_returns["XAU*DXY"] = log_returns["XAU Curncy"] * log_returns["DXY Curncy"]
log_returns["XAU*UST"] = log_returns["XAU Curncy"] * log_returns["USGG10Y Index"]
log_returns["DXY*UST"] = log_returns["DXY Curncy"] * log_returns["USGG10Y Index"]
...
# Reconstruct NAVs from Log Returns
...
output_path = os.path.join(nav_returns_dir, "synthetic_outputs_gold.csv")
...
plt.title(f"GLD NAV Prediction | Log Returns + Interactions (RidgeCV) (R² = {r2_value:.4f})", fontsize=12)
...
# Save the plot
plot_path = os.path.join(current_dir, "nav_comparison_gold.png")
