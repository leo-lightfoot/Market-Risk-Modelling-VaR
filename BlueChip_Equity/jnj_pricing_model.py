...
file_path = os.path.join(project_root, "Raw_Data", "combined_data_jnj.csv")
...
target_column = "JNJ US Equity"
factor_columns = ["S&P 500 Index", "VIX Index", "USD Index", "Treasury Rate Proxy", 
                 "XLV Sector Index", "JNJ P/E Ratio", "DIVIDEND_YIELD"]
...
log_returns["S&P*XLV"] = log_returns["S&P 500 Index"] * log_returns["XLV Sector Index"]
...
log_returns["P/E*XLV"] = log_returns["JNJ P/E Ratio"] * log_returns["XLV Sector Index"]
log_returns["P/E*S&P"] = log_returns["JNJ P/E Ratio"] * log_returns["S&P 500 Index"]
log_returns["P/E*VIX"] = log_returns["JNJ P/E Ratio"] * log_returns["VIX Index"]
log_returns["DIV*S&P"] = log_returns["DIVIDEND_YIELD"] * log_returns["S&P 500 Index"]
log_returns["DIV*XLV"] = log_returns["DIVIDEND_YIELD"] * log_returns["XLV Sector Index"]
...
output_path = os.path.join(nav_returns_dir, "synthetic_outputs_jnj.csv")
...
plt.title(f"JNJ NAV Prediction | Log Returns + Interactions (RidgeCV) (R² = {r2_value:.4f})", fontsize=12)
...
# Save the plot
plot_path = os.path.join(current_dir, "nav_comparison_jnj.png")
